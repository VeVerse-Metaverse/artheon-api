...
class CRUDEvent(CRUDEntity[models.Event, schemas.EventCreate, schemas.EventUpdate]):
    imageService = inject.attr(Service)

    # Filter out mods without files.
    def index(self, db, *, requester, offset=0, limit=10, filters=None, options=None) -> EntityBatch[models.Event]:
    ...
    def create_for_requester(self, db: Session, *, requester: models.User, source: schemas.EventCreate, unique_fields=None) -> models.Event:
    ...
    def get_create_required_fields() -> List[str]:
        return [models.Event.title.name]
